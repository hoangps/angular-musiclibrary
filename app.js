...
myApp.controller('mainController', function($scope, $http, $location, anchorSmoothScroll, $interval){
    ...
    $scope.showList = function(listType, listName){
        ...
        $http({
            method: 'GET',
            url: 'http://api.soundcloud.com/users/' + $scope.userProfile.id + '/' + listType + '?client_id=' + CLIENT_ID
        }).then(function(res){
    ...
    $scope.getTrackCurrentPercent = function(){
        ...
        $scope.player.trackElapsedPercentageStyle = {"width" : (currentTime*100/totalTime).toFixed(2) + "%"};
    }
